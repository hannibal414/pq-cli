...
class ChooseCharacterView(MenuView):
    def __init__(self, screen: T.Any, roster: Roster, title: str) -> None:
        ...
        for y, player in enumerate(roster.players, 1):
            ...
            best_equip = (
                player.equipment.best.render()
                if player.equipment.best is not None
                else "-"
            )

            best_spell = player.spell_book.best
            best_spell_name = (
                (_(best_spell.name) + " " + to_roman(best_spell.level))
                if best_spell
                else "-"
            )

            best_stat_name = (
                f"{_(player.stats.best_prime.value)} "
            ...
            label = (
                ...
                + _("{name} the {race} ({act})").format(
                    name=player.name,
                    race=_(player.race.name),
                    act=act_name(player.quest_book.act).render(),
                )
                + "\n    "
                + _("Level {level} {class_}").format(
                    level=player.level, class_=_(player.class_.name)
                )
